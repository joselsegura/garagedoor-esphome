...
class GPIOSafeguard:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if actual_value == value:
                    if attempt > 0:
                        self.logger.info(f"🔧 CORRECTED: Set GPIO{pin} to {value} (verified on attempt {attempt + 1})")
                    else:
                        self.logger.info(f"🔧 CORRECTED: Set GPIO{pin} to {value} (verified)")
                    return True
                elif actual_value is None:
                    self.logger.warning(f"⚠️  Set GPIO{pin} to {value} but could not verify (read failed) - attempt {attempt + 1}")
                    if attempt < max_retries - 1:
                        time.sleep(0.05)  # Wait a bit longer before retry
                        continue
                    return False  # Treat read failure as set failure for safety
                else:
                    self.logger.warning(f"⚠️  Set GPIO{pin} to {value} but read back {actual_value} - attempt {attempt + 1}")
                    if attempt < max_retries - 1:
                        time.sleep(0.05)  # Wait a bit before retry
                        continue
                    else:
                        self.logger.error(f"❌ Set GPIO{pin} to {value} but read back {actual_value} after {max_retries} attempts - GPIO change failed!")
                        return False
                
            except subprocess.CalledProcessError as e:
                self.logger.warning(f"⚠️  Failed to set GPIO{pin} to {value} (attempt {attempt + 1}): {e}")
                if attempt < max_retries - 1:
                    time.sleep(0.05)  # Wait before retry
                    continue
                else:
                    self.logger.error(f"❌ Failed to set GPIO{pin} to {value} after {max_retries} attempts")
                    return False
    ...
    def initialize_gpios(self):
        ...
        for pin in MONITORED_PINS:
            ...
            if current_state != 1:
                self.logger.warning(f"⚠️  GPIO{pin} is not in safe state (currently {current_state})")
        ...
        for pin in MONITORED_PINS:
            ...
            if previous_state != current_state:
                ...
                if current_state == 0:
                    ...
                    self.logger.info(f"⏱️  GPIO{pin} timer started (max {SAFETY_RULES[pin]}s allowed)")
                    ...
                    if self.gpio_timers[pin] is not None:
                        ...
                        self.logger.info(f"✅ GPIO{pin} returned to safe state after {duration:.3f}s")
                    self.gpio_timers[pin] = None
            ...
            if current_state == 0 and self.gpio_timers[pin] is not None:
                ...
                if elapsed > max_allowed:
                    ...
                    if self.set_gpio(pin, 1):
                        ...
                        self.gpio_timers[pin] = None
                        self.logger.warning(f"🛡️  SAFETY: GPIO{pin} forced to safe state (1) - correction verified")
                    else:
                        self.logger.error(f"❌ CRITICAL: Failed to correct GPIO{pin}! Verification failed - manual intervention may be required!")
        ...
        try:
            while True:
                ...
                if (current_time - last_status_time).total_seconds() >= status_interval:
                    ...
                    self.logger.info(f"📊 Status: {status} | Violations: {self.violations_count} | Corrections: {self.corrections_made}")
...
def main():
    ...
    parser.add_argument('--dry-run', action='store_true', 
                       help='Run in simulation mode (no actual GPIO changes)')
