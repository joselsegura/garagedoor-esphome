...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('ha_test_results.log'),
...
class HomeAssistantTester:
    ...
    def test_cover_service(self, entity_id: str, service: str, test_id: str, description: str) -> bool:
    ...
    def test_switch_service(self, entity_id: str, service: str, test_id: str, description: str) -> bool:
    ...
    def test_all_states(self, test_id: str, description: str) -> bool:
        ...
        if success:
            logger.info(f"✅ {test_id} passed ({response_time:.3f}s) - Found {entity_count} entities")
        else:
            logger.error(f"❌ {test_id} failed: HTTP {status_code} ({response_time:.3f}s)")
            logger.error(f"   Response: {response_data}")
    ...
    def run_basic_cover_tests(self) -> int:
        ...
        if not self.test_cover_service("cover.garage_door", "open_cover", "TC-001", "Open main garage door"):
            failed_tests += 1
        time.sleep(2)  # Brief delay between tests
        
        if not self.test_cover_service("cover.garage_door", "stop_cover", "TC-003", "Stop main garage door"):
            failed_tests += 1
        time.sleep(2)
        
        if not self.test_cover_service("cover.garage_door", "close_cover", "TC-002", "Close main garage door"):
            failed_tests += 1
        time.sleep(2)
        
        # Internal garage door tests
        if not self.test_cover_service("cover.internal_garage_door", "open_cover", "TC-004", "Open internal garage door"):
            failed_tests += 1
        time.sleep(2)
        
        if not self.test_cover_service("cover.internal_garage_door", "stop_cover", "TC-006", "Stop internal garage door"):
            failed_tests += 1
        time.sleep(2)
        
        if not self.test_cover_service("cover.internal_garage_door", "close_cover", "TC-005", "Close internal garage door"):
            failed_tests += 1
    ...
    def run_basic_switch_tests(self) -> int:
        ...
        for entity_id, description in switches:
            # Test turn on
            if not self.test_switch_service(entity_id, "turn_on", f"TC-{test_counter:03d}", f"Turn on {description}"):
                failed_tests += 1
            time.sleep(1)
            
            # Test turn off
            if not self.test_switch_service(entity_id, "turn_off", f"TC-{test_counter+1:03d}", f"Turn off {description}"):
                failed_tests += 1
            time.sleep(1)
    ...
    def run_state_tests(self) -> int:
        ...
        for entity_id, description in entities:
            if not self.test_entity_state(entity_id, f"TC-016-{entity_id.split('.')[-1]}", description):
                failed_tests += 1
            time.sleep(0.5)
    ...
    def run_error_tests(self) -> int:
        ...
        if status_code == 200:
            ...
            logger.info(f"✅ ETC-001 passed: Correctly rejected invalid entity (HTTP {status_code})")
        ...
        if status_code == 200:
            ...
            logger.info(f"✅ ETC-003 passed: Correctly rejected invalid service (HTTP {status_code})")
...
def main():
    parser = argparse.ArgumentParser(description='Home Assistant API Test Runner for Garage Door Controller')
    ...
    parser.add_argument('--states-only', action='store_true', help='Run only state verification tests')
